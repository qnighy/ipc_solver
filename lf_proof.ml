...
let rec pp_print_proof_internal env anum pnum ant sucL sucR ppf pr =
  ...
  | LF_RD1L pr ->
      ...
      | _ -> raise (Invalid_argument "LF_RD1L given, but not POr")
      end
  | LF_RD1R pr ->
      begin match sucR with
      | POr (t1,t2) ->
      ...
      | _ -> raise (Invalid_argument "LF_RD1R given, but not POr")
      end
  | LF_RD2L pr ->
      begin match sucR with
      | POr (t1,t2) ->
          fprintf ppf "[\\/R2L]:@,%a@,"
            (pp_print_proof_internal env anum pnum ant sucL t1) pr
      | _ -> raise (Invalid_argument "LF_RD2L given, but not POr")
      end
  | LF_RD2R pr ->
      begin match sucR with
      | POr (t1,t2) ->
          fprintf ppf "[\\/R2R]:@,%a@,"
            (pp_print_proof_internal env anum pnum ant sucL t2) pr
      | _ -> raise (Invalid_argument "LF_RD2R given, but not POr")
      end
  | LF_RI pr ->
      begin match sucR with
      | PArrow (t1,t2,_) ->
  ...
  | LF_LIC (x,pr) ->
      let (ant0,t,ant1) = cutAnt ant x in
      begin match t with
      | PArrow (PAnd (t1,t2,_),t3,_) ->
          fprintf ppf "[->/\\L]:@,%a@,"
            (pp_print_proof_internal env (anum+1) pnum
              (ant0@(PArrow (t1,PArrow (t2,t3,false),false),anum)::ant1) sucL sucR) pr
  ...
  | LF_LII (x,pr1,pr2) ->
      ...
      | PArrow (PArrow (t1,t2,_),t3,_) ->
          fprintf ppf "[->->L]:@,%a@,%a@,"
            ...
            | Some sucLS ->
                pp_print_proof_internal env (anum+2) pnum
                  (ant0@(PArrow (sucR,sucLS,false),anum)::(t1,anum+1)::ant1)
                  (Some t3) t2
            end pr1
            (pp_print_proof_internal env (anum+1) pnum
